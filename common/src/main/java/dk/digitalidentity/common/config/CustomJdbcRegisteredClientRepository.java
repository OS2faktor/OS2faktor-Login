...
// CUSTOM: Changes to original class marked with "CUSTOM" comment (also removed asserts in DAO operations, because we then never get NULL returns)
/**
...
public class CustomJdbcRegisteredClientRepository implements RegisteredClientRepository {
	...
	private static final String UPDATE_REGISTERED_CLIENT_SQL = "UPDATE " + TABLE_NAME
			+ " SET client_secret = ?, client_name = ?, client_authentication_methods = ?, authorization_grant_types = ?," // CUSTOM: Add client_secret to update statement
...
//		parameters.remove(0); // remove client_secret CUSTOM: We want to be able to update the secret
		parameters.remove(1); // remove client_secret_expires_at
		parameters.add(id);
