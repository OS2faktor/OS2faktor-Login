...
public class CoreDataService {
    ...
    @SneakyThrows
    public void sendKombitRoleDataFull(CoreDataFullJfr coreData) throws RestClientResponseException {
        // Determine URL
        String baseUrl = configuration.getCoreData().getBaseUrl();
        String url = baseUrl + "/api/coredata/jfr/full";

        // Add headers
        HttpHeaders headers = new HttpHeaders();
        headers.set("apiKey", configuration.getCoreData().getApiKey());
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<CoreDataFullJfr> request = new HttpEntity<>(coreData, headers);
    ...
    @SneakyThrows
    public void sendKombitRoleDataDelta(CoreDataDeltaJfr coreData) throws RestClientResponseException {
        // Determine URL
        String baseUrl = configuration.getCoreData().getBaseUrl();
        String url = baseUrl + "/api/coredata/jfr/delta";

        // Add headers
        HttpHeaders headers = new HttpHeaders();
        headers.set("apiKey", configuration.getCoreData().getApiKey());
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<CoreDataDeltaJfr> request = new HttpEntity<>(coreData, headers);
