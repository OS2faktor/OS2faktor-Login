...
@Slf4j
public class AuditlogApiSecurityFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
		...
		if (authHeader != null) {
			...
			if (!StringUtils.hasLength(configuration.getAuditLog().getApiKey())) {
				apiKeyMatch = false;
			}
			else if (Objects.equals(configuration.getAuditLog().getApiKey(), authHeader)) {
				apiKeyMatch = true;
			}
			
			if (!apiKeyMatch) {
				unauthorized(response, "Invalid ApiKey header (Auditlog)", authHeader);
				return;
			}
			...
			unauthorized(response, "Missing ApiKey header (Auditlog)", authHeader);
		}
	}
