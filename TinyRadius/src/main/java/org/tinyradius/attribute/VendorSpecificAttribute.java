...
public class VendorSpecificAttribute extends RadiusAttribute {
	...
	public void setDictionary(Dictionary dictionary) {
		...
		for (Iterator<RadiusAttribute> i = subAttributes.iterator(); i.hasNext();) {
	...
	public void readAttribute(byte[] data, int offset, int length) throws RadiusException {
		...
		subAttributes = new ArrayList<RadiusAttribute>(count);
		pos = 0;
		while (pos < vsaLen) {
	...
	public String toString() {
		...
		for (Iterator<RadiusAttribute> i = getSubAttributes().iterator(); i.hasNext();) {
