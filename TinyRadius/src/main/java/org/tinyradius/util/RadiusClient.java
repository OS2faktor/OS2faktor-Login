...
public class RadiusClient {
	...
	public synchronized RadiusPacket authenticate(AccessRequest request) throws IOException, RadiusException {
		if (log.isInfoEnabled())
			log.info("send Access-Request packet: " + request);

		RadiusPacket response = communicate(request, getAuthPort());
		if (log.isInfoEnabled())
			log.info("received packet: " + response);
	...
	public synchronized RadiusPacket account(AccountingRequest request) throws IOException, RadiusException {
		if (log.isInfoEnabled())
			log.info("send Accounting-Request packet: " + request);
					...
					if (i == getRetryCount()) {
						if (log.isErrorEnabled()) {
							if (ioex instanceof SocketTimeoutException)
								log.error("communication failure (timeout), no more retries");
							else
								log.error("communication failure, no more retries", ioex);
						}
						throw ioex;
					}
					if (log.isInfoEnabled())
						log.info("communication failure, retry " + i);
