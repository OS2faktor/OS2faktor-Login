...
public abstract class ServiceProvider {
    ...
    public PublicKey getPublicKey(UsageType usageType) throws ResponderException, RequesterException {
    ...
    public SingleLogoutService getLogoutResponseEndpoint() throws ResponderException, RequesterException {
        ...
        if (match.isEmpty()) {
            match = ssoDescriptor
                    .getSingleLogoutServices()
                    .stream()
                    .filter(sloService -> SAMLConstants.SAML2_REDIRECT_BINDING_URI.equals(sloService.getBinding()) && !StringUtils.isEmpty(sloService.getResponseLocation()))
                    .findFirst();
        }

        if (match.isEmpty()) {
            throw new RuntimeException("Could not find Post or Redirect SLO Response endpoint in metadata"); //TODO seems weird
        }
    ...
    public X509Certificate getX509Certificate(UsageType usageType) throws ResponderException, RequesterException {
        ...
        if (match.isEmpty()) {
            throw new RequesterException("Kunne ikke finde tjenesteudbyderens 'X509Certificate' af typen: " + usageType);
        }
    ...
    public abstract Map<String, String> getAttributes(Person person);
