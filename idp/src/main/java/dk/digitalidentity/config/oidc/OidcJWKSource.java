...
public class OidcJWKSource implements JWKSource<SecurityContext> {
	...
	private KeyPair getKeyPair() throws CertificateException, IOException, NoSuchAlgorithmException, KeyStoreException, UnrecoverableKeyException {
		KeyStore ks = keystoreService.getPrimaryKeystoreForIdp();
		String alias = ks.aliases().nextElement();

		Key key = ks.getKey(alias, keystoreService.getPrimaryKeystoreForIdpPassword().toCharArray());
